...
import { MonthlySnapshot, UsdAccount } from "@/lib/types";
import { formatMonth, formatUSD } from "@/lib/calculations";
...
function USDForm() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/snapshots/month/${month}`)
      ...
      .then((data: MonthlySnapshot | null) => {
        setSnapshot(data);
        if (data && data.usdAccounts.length > 0) {
          setAccounts(data.usdAccounts.map((a: UsdAccount) => ({ id: a.id, name: a.name, amount: String(a.amount) })));
        } else {
  ...
  function addRow() { setAccounts([...accounts, { name: "", amount: "" }]); }
  function removeRow(i: number) { setAccounts(accounts.filter((_, idx) => idx !== i)); }
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    if (!snapshot) return;
    setSaving(true);
    const valid = accounts.filter((a) => a.name.trim() && a.amount);
    await fetch(`/api/snapshots/${snapshot.id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        ...snapshot,
        usdAccounts: valid.map((a) => ({ name: a.name.trim(), amount: parseFloat(a.amount) || 0 })),
      }),
    });
    setSaving(false);
    setSaved(true);
  ...
  return (
    <div className="p-4 lg:p-8 max-w-2xl">
      <h1 className="text-2xl font-bold mb-1">Saldos USD</h1>
      ...
      <form onSubmit={handleSave} className="bg-white rounded-xl p-6 shadow-sm border border-slate-100 space-y-4">
        ...
        <div className="pt-2 border-t border-slate-100 flex justify-between items-center">
          <span className="text-sm text-slate-600">Total: <strong>{formatUSD(total)}</strong></span>
          <button type="submit" disabled={saving} className="bg-blue-600 hover:bg-blue-500 text-white font-medium px-6 py-2 rounded-lg transition-colors disabled:opacity-50">
...
export default function LoadUSDPage() {
  return <Suspense fallback={<div className="p-6 text-slate-400">Cargando...</div>}><USDForm /></Suspense>;
}
